...
test("prompt anchor spacer ignores its currently rendered height", async () => {
  const { getPromptAnchorSpacerHeight } = await loadSubject();
  const initial = getPromptAnchorSpacerHeight(400, 900, 200, 600);
  const afterRender = getPromptAnchorSpacerHeight(400, 700 + initial, initial, 600);
...
test("prompt anchor spacer clamps at zero once content can reach the target", async () => {
  const { getPromptAnchorSpacerHeight } = await loadSubject();
  assert.equal(getPromptAnchorSpacerHeight(100, 1000, 250, 600), 0);
  assert.equal(getPromptAnchorSpacerHeight(-20, 200, 0, 600), 0);
});

test("prompt anchor spacer rounds fractional layout measurements up", async () => {
  const { getPromptAnchorSpacerHeight } = await loadSubject();
  assert.equal(getPromptAnchorSpacerHeight(400.25, 700, 0, 600), 301);
});
