...
test("RPC wrapper treats powershell as a builtin tool when preserving extension tools", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  assert.match(helperSource, /"powershell"/);
  assert.match(helperSource, /filter\(\(name\) => !codingToolNames\.has\(name\)\)/);
});
