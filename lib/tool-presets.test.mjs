...
test("ignores the optional powershell builtin when inferring presets", () => {
  const optionalBuiltins = ["powershell"];

  assert.equal(getPresetFromTools(toolEntries(["powershell"], optionalBuiltins)), "default");
  assert.equal(
    getPresetFromTools(toolEntries([...PRESET_DEFAULT, "powershell"], optionalBuiltins)),
    "default",
  );
  assert.equal(
    getPresetFromTools(toolEntries([...PRESET_FULL, "powershell"], optionalBuiltins)),
    "full",
  );
});
